...
export default function CourseUpload({ className }: { className?: string }) {
  ...
  const createCourse = async () => {
    try {
      const response = await axios.post('http://localhost:5001/create-course', courseDetails);
  ...
  const saveCourseStructure = async (courseId: string) => {
    try {
      await axios.post('http://localhost:5001/save-course-structure', {
        courseId,
        chapters
      });
    } catch (error) {
  ...
  const handlePromoVideoUpload = async (file: File) => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/upload-promo-video', formData, {
  ...
  const handleThumbnailUpload = async (file: File) => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/upload-thumbnail', formData, {
        onUploadProgress: (progressEvent) => {
          const progress = Math.round((progressEvent.loaded * 100) / (progressEvent.total ?? 1));
          setPromoUploadProgress({
            progress,
  ...
  const handleBannerUpload = async (file: File) => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/upload-banner', formData, {
        onUploadProgress: (progressEvent) => {
          const progress = Math.round((progressEvent.loaded * 100) / (progressEvent.total ?? 1));
          setPromoUploadProgress({
            progress,
  ...
  const handleSubtopicVideoUpload = async (
    ...
    try {
      const response = await axios.post('http://localhost:5001/upload-subtopic-video', formData, {
        onUploadProgress: (progressEvent) => {
          const progress = Math.round((progressEvent.loaded * 100) / (progressEvent.total ?? 1));
          setChapters(prev => prev.map(chapter =>
            chapter.id === chapterId
